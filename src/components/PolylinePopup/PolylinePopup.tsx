...
import { getSelectedPolyline, findSelectedPolyline } from '../../state/routes/routesReducer';
import { deselectPolyline, overrideAnalysis } from '../../state/routes/routesActions';
        ...
        <Polyline ref={ref} positions={selectedPolylineData.latLngs}>
            <Popup closeOnClick={false} onClose={() => dispatch(deselectPolyline())}>
                {selectedPolylineData.type === "offtrack" && (
                    <button onClick={() => dispatch(overrideAnalysis(selectedPolyline.id, selectedPolyline.analysis_id, "ontrack"))}>
                        Zalicz jako w trasie
                    </button>
                )}

                {selectedPolylineData.type === "ontrack" && (
                    <button onClick={() => dispatch(overrideAnalysis(selectedPolyline.id, selectedPolyline.analysis_id, "offtrack"))}>
