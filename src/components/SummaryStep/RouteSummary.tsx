...
const RouteData = ({ route }: { route: GPX }) => {
    ...
    const routeLength = getFormattedDurationLength(moment(get_start_time), moment(get_end_time));
...
const getFormattedDurationLength = (start: moment.Moment, end: moment.Moment) => {
    const diff = end.diff(start);
    return moment.utc(diff).format('HH:mm:ss');
};

const RouteAnalysisTable = ({ analysis }: { analysis: RouteFragments }) => {
    const offtrackIntervals = useMemo(() => calculateOfftrackIntervals(analysis), [analysis]);
    const offtrackIntervalsSum = useMemo(() => sumOfftrackIntervals(offtrackIntervals), [offtrackIntervals]);
                ...
                {offtrackIntervals.map((interval, index) => {
                    ...
                    return (
                        <TableRow key={index}>
                            ...
                            <TableDataCell>{ getFormattedDurationLength(start, end) }</TableDataCell>
                        </TableRow>
                    );
                })}
